...
By default, it uses a curated list of JSON-RPC endpoints from [DefiLlama/chainlist](https://github.com/DefiLlama/chainlist) against the known deployments of the ethr-did-registry.
...
Use multiple URLs to increase resilience. Internally, it creates a `FailoverProvider` that tries each URL in sequence on failure.
...
Reads `generated/rpcUrls.json` by default. Each chain gets a `FailoverProvider` that cycles through its RPC URLs — if one fails, the next is tried automatically.
...
Fetches RPC candidates from [DefiLlama/chainlist](https://github.com/DefiLlama/chainlist). Returns `Record<number, RpcCandidate[]>`.

### `testRpcUrl(chainId, url, registry, timeout?, testBlocks?)`

Tests a single RPC URL by sending `eth_blockNumber` then `eth_getLogs` at the chain's test block. Returns `RpcTestResult`.

### `FailoverProvider` / `buildFailoverProvider(chainId, urls)`

A `JsonRpcProvider` subclass that tries each URL in sequence on failure. Skips past URLs that return empty logs for historical queries (indicating limited archival access).
...
Generated files are checked in so consumers don't need to probe at install time. Re-run `pnpm probe` periodically to refresh URLs.
